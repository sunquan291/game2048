...
{
    @Override
    public Bitmap blur(Bitmap original, float rad, int t_width, int t_height)
    {
        int radius = (int) rad;
        if (radius < 1)
            radius = 1;
        ...
        float scaleWidth = ((float) original.getWidth()) / t_width;
        float scaleHeight = ((float) original.getHeight()) / t_height;
        matrix.postScale(scaleWidth, scaleHeight);
        return Bitmap.createBitmap(currentPixels, _width, _height, original.getConfig());

        // return Bitmap.createBitmap(currentPixels, t_width,t_height,
        // original.getConfig());
    }
}
